...
// Example specific metadata type (you would define more as needed for strong typing)
interface AircraftGeneralInfoMetadata extends BaseMetadata {
  ...
  parts_cost_hulls?: number;
  parts_cost_weapon_systems?: number;
  parts_cost_engines?: number;
}
...
function formatRetrievedContext(matches: ScoredPineconeRecord<BaseMetadata>[]): string {
  ...
  return matches.map((match, index) => {
    ...
    if (metadata.entity_type === 'aircraft') {
      const acMeta = metadata as AircraftGeneralInfoMetadata; // Cast for easier access
    ...
    //   if (metadata.info_type === 'tank_general_info') {
    //     details += `Crew: ${metadata.crew_size || "N/A"}\n`;
    //   }
    // }
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    console.log('Querying Pinecone with topK=3 for initial semantic matches...'); // Reduced topK for initial
    ...
    if (finalMatches.length > 0 && finalMatches[0].metadata) {
      ...
      const knownItems = ["P-51 Mustang", "MiG-29 Fulcrum"]; // <<< EXPAND THIS LIST as you add more items
        ...
        try {
          ...
          if (overviewRecord) {
            const fetchedMatch: ScoredPineconeRecord<BaseMetadata> = {
              ...
              score: 1.0, // Indicate it was directly fetched / high relevance
              metadata: (overviewRecord.metadata as BaseMetadata) || {},
              // values: overviewRecord.values // Not needed for context string
            };
            finalMatches.unshift(fetchedMatch); 
            console.log(`Successfully fetched and added ${overviewFullTextId} to context.`);
          } else {
              console.log(`Could not fetch record for ${overviewFullTextId}. It might not exist or ID is incorrect.`);
          }
    ...
    // Remove duplicates by ID, keeping the one that appears first (which would be the fetched overview if added)
    const uniqueMatches = Array.from(new Map(finalMatches.map(match => [match.id, match])).values());
    // Limit total context chunks to avoid overly long prompts, e.g., max 5 chunks total
    const limitedMatches = uniqueMatches.slice(0, 5);


    const contexts = formatRetrievedContext(limitedMatches);
    
    console.log('--- COMBINED CONTEXT FOR OPENAI (first 2000 chars) ---'); // Increased log length
...
- If you're asked to list items in War Tycoon, only list the types of items that are requested and actually in the game. Make sure you don't give details, only provide a bulleted list of the types of items requested.
